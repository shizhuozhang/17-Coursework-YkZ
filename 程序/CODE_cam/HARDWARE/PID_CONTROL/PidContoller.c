...
PID_IncrementType  PID_Roll,PID_Yaw;	//外环：角度PID环
...
{	
	...
	PID_Roll.Kp=20;//19,0.11
	PID_Roll.Ti=0.25;/////tiaohao
	PID_Roll.Td=0;
	...
	Increment_PID(&PID_Roll);//执行绝对式PID算法
	Increment_PID(&PID_Yaw);//执行绝对式PID算法
	
	Motor_Value[0]=(s16)PID_Roll.CtrOut;
	Motor_Value[1]=(s16)PID_Yaw.CtrOut;	
...
void Motor_Drive(s16 To_Motor[2])
{
	...
	TIM4->CCR1-=To_Motor[0];//Roll	
	TIM4->CCR2-=To_Motor[1];//偏航yaw
//输出限幅和死区限位
	if((TIM4->CCR1)>6000)		
		TIM4->CCR1=6000;
